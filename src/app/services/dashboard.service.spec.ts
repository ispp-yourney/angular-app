...
describe('DashboardService', () => {
  ...
  it('should use dashboard http get requests', () => {
    const buscadorObservable: Observable<any> = of(itinerarySearchResult);
    httpClientMock.get.and.returnValue(buscadorObservable);
    service.itinerariosOrdenadosPorVisualizaciones("España", "Sevilla")
      .subscribe(buscadorResponse => {
        expect(buscadorResponse.content[0].name).toEqual("Sevilla, tierra de pasión")
      });

    service.landmarkOrdenadosPorVisualizaciones("España", "Sevilla")
      .subscribe(buscadorResponse => {
        expect(buscadorResponse.content[0].name).toEqual("Sevilla, tierra de pasión")
      });

    service.itinerariosOrdenadosPorComentarios("España", "Sevilla")
      .subscribe(buscadorResponse => {
        expect(buscadorResponse.content[0].name).toEqual("Sevilla, tierra de pasión")
      });

    service.itinerariosOrdenadosPorValoracion("España", "Sevilla")
      .subscribe(buscadorResponse => {
        expect(buscadorResponse.content[0].name).toEqual("Sevilla, tierra de pasión")
      });

    service.ultimosItinerariosOrdenadosPorComentarios("España", "Sevilla")
      .subscribe(buscadorResponse => {
        expect(buscadorResponse.content[0].name).toEqual("Sevilla, tierra de pasión")
      });

    service.ultimosItinerariosOrdenadosPorValoracion("España", "Sevilla")
      .subscribe(buscadorResponse => {
        expect(buscadorResponse.content[0].name).toEqual("Sevilla, tierra de pasión")
      });
  });
